...
import { CheckCircle2, Clock, Eye, Link, RotateCcw, XCircle } from 'lucide-react';
...
export default function MatchingTest({ 
  ...
  const [connectorLines, setConnectorLines] = useState<Array<{ from: DOMRect; to: DOMRect; isCorrect?: boolean }>>([]);
  ...
  // Handle drag start
  const handleDragStart = (e: React.DragEvent, item: string, type: 'word' | 'meaning', index: number) => {
    if (isCompleted || showResult) return;
  ...
  // Handle drop
  const handleDrop = (e: React.DragEvent, dropItem: string, dropType: 'word' | 'meaning') => {
    e.preventDefault();
  ...
  const allCorrect = showResult && Object.keys(correctMatches).every(word => 
    matches[word] === correctMatches[word]
  );
      ...
      `}>
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-white font-bold text-lg lg:text-xl">مطابقة</h3>
          ...
          {timeLeft !== undefined && timeLeft > 0 && !showResult && (
            <div className={`flex items-center space-x-2 px-4 py-2 rounded-xl ${
              timeLeft <= 15 ? 'bg-red-900/30 text-red-400' : 'bg-purple-900/30 text-purple-400'
            }`}>
              <Clock size={18} />
        ...
        <div className="bg-gray-700/30 rounded-2xl p-4 lg:p-6 border border-gray-600/50">
          <p className="text-gray-300 text-center">
            {!showResult ? (
              <>
                <span className="hidden lg:inline">اسحب الكلمات إلى معانيها أو </span>
        ...
        <div className="mb-6 text-center">
          <div className="inline-flex items-center space-x-3 bg-gray-800/50 rounded-2xl px-6 py-3 border border-gray-700">
            ...
            <span className={`font-bold text-lg ${
              matchedCount === totalCount ? 'text-green-400' : 'text-purple-400'
            }`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 relative">
          ...
          <div className="space-y-4">
            ...
            {words.map((word, index) => {
              ...
              if (showResult && isMatched) {
                ...
                resultClass = isCorrect ? 'border-green-500 bg-green-900/20' : 'border-red-500 bg-red-900/20';
              }
              ...
              return (
                <div
                  ...
                  className={`
                    ...
                    ${isSelected ? 'border-blue-500 bg-blue-900/30 shadow-lg scale-[1.02]' : ''}
                    ${isMatched && !isSelected ? 'border-purple-500 bg-purple-900/20' : ''}
                    ${!isMatched && !isSelected ? 'border-gray-700 bg-gray-800/50 hover:border-gray-600 hover:bg-gray-800/70' : ''}
                    ${resultClass}
          ...
          <div className="space-y-4">
            ...
            {meanings.map((meaning, index) => {
              ...
              const isDragOver = dragState.isDragging && dragState.dragType === 'word';
              ...
              return (
                <div
                  ...
                  className={`
                    p-4 lg:p-6 rounded-2xl border-2 transition-all duration-300 cursor-pointer
                    transform hover:scale-[1.02] active:scale-[0.98] touch-manipulation
                    ${isDragOver ? 'border-blue-400 bg-blue-900/20 scale-[1.02]' : ''}
                    ${isMatched ? 'border-green-500 bg-green-900/20' : 'border-gray-700 bg-gray-800/50 hover:border-gray-600 hover:bg-gray-800/70'}
                    ${resultClass}
                    ${showResult ? 'cursor-default' : ''}
                  `}
                  style={{ 
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    {/* Match Status */}
                    <div className="mr-3">
                      {showResult && isMatched && matchedWord && (
          ...
          <div className="flex items-center space-x-4 mb-4">
            {allCorrect ? (
              ...
              <>
                ...
                <div>
                  <h4 className="text-orange-400 font-bold text-xl">يمكنك التحسن</h4>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="text-center">
              <div className="text-2xl font-bold text-green-400">
                {Object.keys(correctMatches).filter(word => matches[word] === correctMatches[word]).length}
              </div>
              <div className="text-gray-400 text-sm">مطابقات صحيحة</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-red-400">
                {Object.keys(matches).filter(word => correctMatches[word] !== matches[word]).length}
              </div>
              <div className="text-gray-400 text-sm">مطابقات خاطئة</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-purple-400">
                {Math.round((Object.keys(correctMatches).filter(word => matches[word] === correctMatches[word]).length / totalCount) * 100)}%
              </div>
