...
const SessionPreview: React.FC<SessionPreviewProps> = ({
  ...
  const sessionStats = useMemo(() => {
    ...
    let timePerWord;
    switch (mode) {
      case 'speed': timePerWord = 0.5; break; // 30 seconds per word
      case 'reading': timePerWord = 0.2; break; // 12 seconds per word
      case 'challenge': timePerWord = 1.5; break; // 90 seconds per word
      case 'reverse': timePerWord = 1.2; break; // 72 seconds per word
      default: timePerWord = 1; // 60 seconds per word
    }
  ...
  if (words.length === 0) {
    return (
      <div className={`bg-orange-900/20 rounded-2xl p-6 border border-orange-800/30 ${className}`}>
        <div className="text-center">
          <AlertCircle className="text-orange-400 mx-auto mb-4" size={32} />
          <h3 className="text-lg font-semibold text-orange-400 mb-2">لا توجد كلمات للدراسة</h3>
  ...
  return (
    <div className={`bg-gray-800 rounded-2xl p-6 border border-gray-700 space-y-6 ${className}`}>
          ...
          {Object.entries(sessionStats.difficultyBreakdown).map(([difficulty, count]) => {
            ...
            return (
              <div key={difficulty} className="flex items-center space-x-3">
                ...
                <div className={`text-sm ${color.text} font-medium w-12 text-right`}>
                  {count}
                </div>
              </div>
            );
      ...
      {Object.keys(sessionStats.categoryBreakdown).length > 1 && (
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {Object.entries(sessionStats.categoryBreakdown).map(([category, count]) => (
              <div 
                key={category}
        ...
        <div className="bg-green-900/20 rounded-xl p-4 border border-green-800/30">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center space-x-2">
              ...
              <h4 className="text-green-400 font-medium">التوقع المتوقع</h4>
            </div>
